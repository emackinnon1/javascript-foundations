...
describe('Medusa', function() {
  ...
  it('should unstone the first victim and stone the 4th coming in', function() {
    var medusa = new Medusa('Martha');
    var victim1 = new Person('Tim');
    var victim2 = new Person('Boris');
    var victim3 = new Person('Donald');
    var victim4 = new Person('Dee');

    // medusa stares at first victim
    medusa.stare(victim1);
    // assert that the first victim is stoned
    assert.equal(medusa.statues[0].name, 'Tim');
    // medusa stares at second victim
    medusa.stare(victim2);
    medusa.stare(victim3);
    medusa.stare(victim4);
    // assert the fourth victim is stoned
    assert.equal(medusa.statues[2].name, 'Dee');
